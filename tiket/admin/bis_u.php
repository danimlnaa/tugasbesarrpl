        ...
        <div class="row">
            <div class="col s10">
                <div class="card-panel">
                    ...
                    $id = $_GET['id_bis'];
                    $sql = mysqli_query($conn, 'select * from bis where id_bis="'.$id.'" ');
                    $dtt = mysqli_fetch_array($sql);
                    ?>
                    <form method="post" action="bis_uu.php?id_bis=<?=$dtt['id_bis'];?>">
                        ...
                        <select name="id_bis" class="browser-default">
                            <option selected disabled>Type: </option>
                            <?php
                                include 'koneksi.php';
                                $result = mysqli_query($conn, "SELECT * FROM type_class ");
                                while ($row = mysqli_fetch_assoc($result)) {?>
                                <option value="<?php echo $row['id_bis']?>"><?php echo $row['description']?></option>
                            <?php }?>
                        </select>
